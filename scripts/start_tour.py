...
MESSAGES: dict[str, dict[str, str]] = {
    "en": {
        ...
        "install_retry_node": "Press Enter after installing Node.js to retry, or type q to exit.",
    },
    "zh": {
        ...
        "install_retry_node": "安装好 Node.js 后按回车重试，或输入 q 退出。",
    },
}
...
def _embedding_provider_options(current: str | None) -> list[tuple[str, str, str]]:
    ...
    common = ["openai", "navy", "jina", "cohere", "ollama", "vllm", "azure_openai", "custom"]
    options: list[tuple[str, str, str]] = []
    for name in common:
...
def _install_dependencies() -> None:
    ...
    if node_version and npm_version:
        ...
        for _ in range(3):
            try:
                answer = input(f"  {_t('install_retry_node')} ").strip().lower()
            except EOFError:
                answer = "q"
